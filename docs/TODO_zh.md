...
## 📅 2026-01-01: Token 经济与 API 深度优化 (v0.2.1)

**目标**: 进一步降低 MCP 服务端的静态 Token 成本，提升大规模项目下的响应性能。

### 1. 工具定义精简 (DONE)
- [x] **手写 Schema**: 移除 Zod 自动生成的冗长定义，手动精简 `definitions.ts` (-49% Token)。
- [x] **隐藏内部工具**: 将 `update_task` 从公开列表中移除。

### 2. 增量读取与渐进加载 (DONE)
- [x] **消息增量读取**: 实现 `read_cursors` 游标表，`read_messages` 自动只返回未读消息。
- [x] **拓扑图渐进加载**: `get_global_topology` 默认返回摘要，支持 `projectId` 深度查询。
- [x] **资源模板优化**: `listResources` 移除动态项目列表，改用 O(1) 的模板模式，通过 `registry` 发现 ID。

### 3. 文档整理 (DONE)
- [x] **目录结构优化**: 根目录仅保留英文核心文档，其余文档移至 `docs/` 并建立互链。
- [x] **助手指南更新**: `ASSISTANT_GUIDE.md` 升级至 v0.2.1，强调"渐进式发现"模式。

---
*最后更新: 2026-01-01 by Antigravity AI.*
